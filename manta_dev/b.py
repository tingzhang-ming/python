...
client = pymanta.MantaClient(
    ...
    signer=signer)
bucket = '/{}/stor/{}'.format(account, "test1/dumps")

# client.put_directory2(os.path.join(bucket, 'm'))
# client.mkdir(bucket, parents=True)
# m = MockStream(1000)
# client.put_object(os.path.join(bucket, "test.txt"), data=m)
# with open('/tmp/a.txt', 'w') as f:
#     f.write('dddddddddddddddddsfdsgfggsdfgdgdfg')
# with open('/tmp/a.txt', 'rb') as f:
#     client.put_object(os.path.join(bucket, "test.txt"), data=f)
# conn, res = client.get_object(os.path.join(bucket, "test.txt"))
# for i in res:
#     print i
# conn.close()
# _, res = client.get_object(os.path.join(bucket, "test1.sql"), stream=False)
# print res
# dirs = client.list_directory(bucket)
# for d in dirs:
#     print d
path = os.path.join(bucket, "test1.sql")
dirs = client.list_directory(os.path.dirname(path))
for d in dirs:
