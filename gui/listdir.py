...
class DirList(object):
    ...
    def __init__(self, initdir=None):
        ...
        self.dirs.bind('<Double-1>', self.setDirAndGo)    # 鼠标左键双击
        self.dirsb.config(command=self.dirs.yview)
        self.dirs.pack(side=LEFT, fill=BOTH)
        self.dirfm.pack()

        self.dirn = Entry(self.top, width=50, textvariable=self.cwd)  # 可输入的框
        self.dirn.bind('<Return>', self.doLS)  # 回车
        self.dirn.pack()
    ...
    def setDirAndGo(self, ev=None):
        ...
        cur = self.dirs.curselection()
        check = None
        if cur:
            check = self.dirs.get(cur)
        if not check:
            check = os.curdir
        self.cwd.set(check)
        self.doLS()
